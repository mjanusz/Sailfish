  ...
  %if node_addressing == 'indirect':
    {
      ...
      if (target_gi != INVALID_NODE) {
        ${dist_out}[target_gi + ${dist_size * idir}] = ${convert}${rhs};
      }
    }
  %else:
    %if shared:
      ${get_odist(dist_out, idir, xoff, yoff, zoff, offset)} = ${convert}(prop_${grid.idx_name[idir]}[lx]);
    %else:
      ${get_odist(dist_out, idir, xoff, yoff, zoff, offset)} = ${convert}(${rhs});
    %endif
  %endif
</%def>
